...
PRESERVE_DIR="$INSTALL_DIR/modules/private"
...
ZIP_URL="https://github.com/$REPOSITORY_OWNER/$REPOSITORY/archive/refs/tags/release/$VERSION.zip"
TMP_DIR=$(mktemp -d)
...
if [[ -d "$INSTALL_DIR" ]]; then
  ...
  mv "$INSTALL_DIR" "$INSTALL_DIR"_"$(date +%d%m%Y_%H%M%S)"
fi
...
ln -sf "$INSTALL_DIR/$REPOSITORY-$VERSION/$REPOSITORY.sh" "$BIN_PATH/$ALIAS"
